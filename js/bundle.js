...
var Game = __webpack_require__(4),
    GameView = __webpack_require__(8);
...
var Constants = __webpack_require__(0),
    Edge = __webpack_require__(5),
    Graph = __webpack_require__(7),
    Util = __webpack_require__(1),
    Vertex = __webpack_require__(2);
...
var Edge = __webpack_require__(5),
    Util = __webpack_require__(1),
    Vertex = __webpack_require__(2);
...
GameView.prototype.renderRules = function () {
  ...
  if (prevRules.length > 0) {
    ...
    var _$rulesModal = $("<div>").addClass("modal").addClass("rules").css({ display: "block" });
    var $rulesContent = $("<div>").addClass("modal-content");
    var $rules = $("<p>").text("Voce pode salvar seu pais de atentados terroristas hackeando. Transforme os grafos em planares!");
    ...
    _$rulesModal.append($rulesContent);

    var $playButton = $("<a>").text("Jogar").addClass("button").addClass("play");

    $rulesContent.append($playButton);
    // $rulesContent.append($definitions);

    $playButton.on("touchstart click", function (event) {
      _$rulesModal.css({ display: "none" });
    });

    _$rulesModal.on("touchstart click", function (event) {
      _$rulesModal.css({ display: "none" });
    });

    this.root.append(_$rulesModal);
  }
};
...
GameView.prototype.renderModal = function () {
  ...
  if (prevModals.length > 0) {
    ...
    var _$modal = $("<div>").addClass("modal").addClass("win-modal").css({ display: "none" });

    var $modalContent = $("<div>").addClass("modal-content").addClass("win-modal-content");
    var $congrats = $("<h2>").text("Parabens, voce hackeou um terrorista e salvou o seu pais de um atentado!");
    ...
    _$modal.append($modalContent);

    var $nextButton = $("<a>").text("Proximo Level").addClass("button").addClass("next-level-modal");
    ...
    $nextButton.on("touchstart click", function (event) {
      ...
      cancelAnimationFrame(_this.refreshIntervalId);
      _this.playLevel();
    });

    _$modal.on("touchstart click", function (event) {
      event.stopPropagation();
      event.preventDefault();
...
GameView.prototype.renderButtons = function () {
  ...
  var $github = $("<a href=\"https://github.com/leodsgn/watch_dogz_graph_game\" target=\"_blank\"><div class=\"button github\"/></a>");

  var $buttonRules = $("<a>").addClass("button").addClass("show-rules").text("Sobre");
...
GameView.prototype.checkPlanarity = function () {
  if (this.game.isPlanar()) {
    ...
    var $stage = $(".stage").empty().append("Estagio: <span>" + (this.stage + 1) + "</span>");
    var $moves = $(".moves").empty().append("Movimentos: <span>" + this.game.moves + "</span>");
...
GameView.prototype.renderGraph = function () {
  ...
  this.game.edges.forEach(function (edge, i) {
    edge.draw(_this3.ctx, _this3.game.edges);
  });

  this.game.vertices.forEach(function (vertex, i) {
    vertex.draw(_this3.ctx);
  });
};
...
GameView.prototype.bindGraphEvents = function () {
  ...
  $("canvas").on("mousedown touchstart", function (event) {
    ...
    if (event.originalEvent.targetTouches) {
      ...
      if (touch) {
        ...
        _this4.currentMousePos.x = touch.pageX + xAdjust - Game.leftOffset;
        _this4.currentMousePos.y = touch.pageY + yAdjust;
        console.log("NEW TOUCH DOWN");
        console.log(_this4.currentMousePos.x + ", " + _this4.currentMousePos.y);
      }
    }

    _this4.game.vertices.forEach(function (vertex) {
      var dist = Util.distFromMouse(vertex, _this4.currentMousePos);
  ...
  $(document).on("mouseup touchend", function (event) {
    ...
    _this4.game.dropVertices();
    _this4.checkPlanarity();
  });
  ...
  $(document).mousemove(function (event) {
    ...
    _this4.currentMousePos.x = event.pageX + xAdjust - Game.leftOffset;
    _this4.currentMousePos.y = event.pageY + yAdjust;
  });
